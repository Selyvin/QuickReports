...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`[qb-time-activities] ${start_date} to ${end_date} for user ${user.id}`)

    const query = `SELECT * FROM TimeActivity WHERE TxnDate >= '${start_date}' AND TxnDate <= '${end_date}'`
    const rows = await qbQueryAll(realm_id, access_token, query)

    // Return the raw QBO QueryResponse shape so the frontend can parse it
    return json({ QueryResponse: { TimeActivity: rows } })
  } catch (e: unknown) {
    console.error('[qb-time-activities] Error:', e)
